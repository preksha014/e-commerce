    ...
    <div class="flex flex-col">
        <div class="relative flex">
            ...
            <div
                ...
                @auth
                    <button onclick="toggleWishlist({{ $product->id }})" class="wishlist-btn flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-amber-400">
...
function toggleWishlist(productId) {
    ...
    .then(data => {
        if (data.success) {
            ...
            wishlistBtn.classList.toggle('bg-red-500');
            wishlistBtn.classList.toggle('bg-amber-400');
        } else {
            alert(data.message);
        }
    })
